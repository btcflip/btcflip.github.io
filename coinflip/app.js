...
if (window.history && window.history.replaceState) {
  window.history.replaceState({}, document.title, "/coinflip");
} else {
...
var caset2;
            ...
            var wagerSatoshis = document.getElementById("betSize").value * 100;
			var targetspan = document.getElementById("betTargetSpan").innerHTML;
			if(targetspan == 'HEADS') { caset2 = 'h'; }
			if(targetspan == 'TAILS') { caset2 = 't'; }
            if (caset2 === 'h') {
...
var caset2;
  ...
  _changeTarget: function() {
	  ...
	  {
		  target = '<';
		  document.getElementById("betTargetSpan").innerHTML = "TAILS";
		  return;
	  }
	  if(target == '<') 
	  {
		  target = '>';
		  document.getElementById("betTargetSpan").innerHTML = "HEADS";
		  return;
	  }
  },
    _validateMultiplier: function(newStr) {
  ...
  socket.on('connect', function() {
    ...
    socket.on('new_bet', function(bet) {
      ...
      if (bet.kind == 'simple_dice') {
        console.log('[weird] received bet from socket that was a simple_dice bet');
        return;
      }
